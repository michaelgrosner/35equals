...
/** Static tag → name mapping for default visible columns. */
...
export const indexColumn = helper.display({
  id: '0',
  header: () => React.createElement(ColumnSettings),
...
export const makeTagColumn = (tag: number) => {
  ...
  return helper.accessor((row) => cellValue(tag, row), {
    ...
    header: () => React.createElement(
      ...
      name,
      ' ',
      React.createElement('span', { className: 'text-muted-foreground/50 font-normal' }, tag)
    ),
    size: 120,
